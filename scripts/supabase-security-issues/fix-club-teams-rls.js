...
async function fixClubTeamsRLS() {
  console.log('🔧 Fixing club_teams RLS security warning...');
  console.log('');

  try {
    // Read the SQL script
    const sqlPath = path.join(process.cwd(), 'scripts', 'fix_club_teams_rls.sql');
    ...
    if (rlsData && rlsData.length > 0) {
      ...
      console.log('✅ club_teams table RLS status:', rlsEnabled ? '✅ Enabled' : '❌ Disabled');
    } else {
      console.log('⚠️  club_teams table not found');
    }
    ...
    console.log('   • Enabled Row Level Security on club_teams table');
    ...
    console.log('   • RLS now properly controls access to club_teams data');
    console.log('   • Public read access for club-team relationships');
    ...
    console.error('   2. Copy and paste the contents of scripts/fix_club_teams_rls.sql');
