...
async function fixCategorySeasonsRLS() {
  console.log('🔧 Fixing category_seasons RLS security warning...');
  console.log('');

  try {
    // Read the SQL script
    const sqlPath = path.join(process.cwd(), 'scripts', 'fix_category_seasons_rls.sql');
    ...
    if (rlsData && rlsData.length > 0) {
      ...
      console.log('✅ category_seasons table RLS status:', rlsEnabled ? '✅ Enabled' : '❌ Disabled');
    } else {
      console.log('⚠️  category_seasons table not found');
    }
    ...
    console.log('   • Enabled Row Level Security on category_seasons table');
    ...
    console.log('   • RLS now properly controls access to category-season data');
    console.log('   • Public read access for competition configuration');
    ...
    console.error('   2. Copy and paste the contents of scripts/fix_category_seasons_rls.sql');
