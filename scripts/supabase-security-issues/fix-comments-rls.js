...
async function fixCommentsRLS() {
  console.log('🔧 Fixing comments RLS security warning...');
  console.log('');

  try {
    // Read the SQL script
    const sqlPath = path.join(process.cwd(), 'scripts', 'fix_comments_rls.sql');
    ...
    if (rlsData && rlsData.length > 0) {
      ...
      console.log('✅ comments table RLS status:', rlsEnabled ? '✅ Enabled' : '❌ Disabled');
    } else {
      console.log('⚠️  comments table not found');
    }
    ...
    console.log('   • Enabled Row Level Security on comments table');
    console.log('   • Created read policy for all authenticated users');
    console.log('   • Created policies for users to manage their own comments');
    ...
    console.log('   • RLS now properly controls access to comment data');
    console.log('   • Users can only modify their own comments');
    console.log('   • Public read access for project collaboration');
    console.log('   • Admins have full access for moderation');
    ...
    console.error('   2. Copy and paste the contents of scripts/fix_comments_rls.sql');
