-- Setup Lineup Database System
-- Run this script in your Supabase SQL editor to create the lineup management system
...
CREATE TRIGGER update_lineup_players_updated_at 
    BEFORE UPDATE ON lineup_players 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_lineup_coaches_updated_at 
    BEFORE UPDATE ON lineup_coaches 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE INDEX IF NOT EXISTS idx_lineup_players_member_id ON lineup_players(member_id);
CREATE INDEX IF NOT EXISTS idx_lineup_coaches_lineup_id ON lineup_coaches(lineup_id);
CREATE INDEX IF NOT EXISTS idx_lineup_coaches_member_id ON lineup_coaches(member_id);
...
DROP POLICY IF EXISTS "Lineups are viewable by everyone" ON lineups;
CREATE POLICY "Lineups are viewable by everyone" ON lineups
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Lineups are insertable by authenticated users" ON lineups;
CREATE POLICY "Lineups are insertable by authenticated users" ON lineups
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Lineups are updatable by authenticated users" ON lineups;
CREATE POLICY "Lineups are updatable by authenticated users" ON lineups
    FOR UPDATE USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Lineups are deletable by authenticated users" ON lineups;
CREATE POLICY "Lineups are deletable by authenticated users" ON lineups
    FOR DELETE USING (auth.role() = 'authenticated');

-- Policies for lineup_players table
DROP POLICY IF EXISTS "Lineup players are viewable by everyone" ON lineup_players;
CREATE POLICY "Lineup players are viewable by everyone" ON lineup_players
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Lineup players are insertable by authenticated users" ON lineup_players;
CREATE POLICY "Lineup players are insertable by authenticated users" ON lineup_players
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Lineup players are updatable by authenticated users" ON lineup_players;
CREATE POLICY "Lineup players are updatable by authenticated users" ON lineup_players
    FOR UPDATE USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Lineup players are deletable by authenticated users" ON lineup_players;
CREATE POLICY "Lineup players are deletable by authenticated users" ON lineup_players
    FOR DELETE USING (auth.role() = 'authenticated');

-- Policies for lineup_coaches table
DROP POLICY IF EXISTS "Lineup coaches are viewable by everyone" ON lineup_coaches;
CREATE POLICY "Lineup coaches are viewable by everyone" ON lineup_coaches
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Lineup coaches are insertable by authenticated users" ON lineup_coaches;
CREATE POLICY "Lineup coaches are insertable by authenticated users" ON lineup_coaches
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Lineup coaches are updatable by authenticated users" ON lineup_coaches;
CREATE POLICY "Lineup coaches are updatable by authenticated users" ON lineup_coaches
    FOR UPDATE USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Lineup coaches are deletable by authenticated users" ON lineup_coaches;
...
GRANT ALL ON lineup_players TO authenticated;
GRANT ALL ON lineup_coaches TO authenticated;
GRANT USAGE ON SCHEMA public TO authenticated;

-- Test the setup
SELECT 'Lineup system setup completed successfully!' as status;
