-- Update training_sessions related functions to use category_id instead of category VARCHAR
-- Run this in your Supabase SQL Editor

-- Step 1: Drop the old function first to avoid conflicts
...
-- Step 3: Drop the old get_attendance_summary function
DROP FUNCTION IF EXISTS get_attendance_summary(VARCHAR(50), UUID, UUID);

-- Step 4: Create new get_attendance_summary function that uses category_id
...
-- Step 5: Drop the old get_attendance_records function (if it exists with different signature)
DROP FUNCTION IF EXISTS get_attendance_records(UUID, UUID);

-- Step 6: Create new get_attendance_records function that uses category_id
