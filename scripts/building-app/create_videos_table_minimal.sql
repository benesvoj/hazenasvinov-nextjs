-- Minimal videos table creation (without optional foreign keys)
-- Use this if clubs or seasons tables don't exist yet
...
CREATE TABLE IF NOT EXISTS videos (
  ...
  club_id UUID, -- No foreign key constraint
  recording_date DATE,
  season_id UUID, -- No foreign key constraint
  thumbnail_url TEXT,
  duration TEXT,
...
COMMENT ON COLUMN videos.club_id IS 'Club this video is related to (optional, no FK constraint)';
COMMENT ON COLUMN videos.recording_date IS 'Date when the video was recorded (optional)';
COMMENT ON COLUMN videos.season_id IS 'Season this video belongs to (optional, no FK constraint)';
