...
```
MembersAdminPage (error.tsx.backup:31)
...
**Issue:** All three member types are fetched simultaneously on page mount (error.tsx.backup:48-54)
```typescript
...
4. **Manual refresh logic:** Context-based refresh is error-prone (error.tsx.backup:96-101)
