...
const CategoryProvider = createContext<CoachCategoryContextType | null>(null);
...
export function CoachCategoryProvider({children}: CoachCategoryProviderProps) {
  ...
  return <CategoryProvider.Provider value={value}>{children}</CategoryProvider.Provider>;
}

export function useCoachCategory(): CoachCategoryContextType {
  const ctx = useContext(CategoryProvider);
