/**
 * Database schema for `category_lineups` table
 *
 * ⚠️ AUTO-GENERATED - DO NOT EDIT MANUALLY
 * Generated by: scripts/split-db-types.js
 * Source: src/types/database/supabase.ts
 *
 * To regenerate:
 *   1. Run database migrations
 *   2. Run: npm run generate:types
 *
 * Last generated: 2025-11-25
 */

export interface CategoryLineupSchema {
  category_id: string;
  created_at: string | null;
  created_by: string;
  description: string | null;
  id: string;
  is_active: boolean | null;
  name: string;
  season_id: string;
  updated_at: string | null;
}

/**
 * Type for INSERT operations (auto-generated fields excluded)
 */
export type CategoryLineupInsert = Omit<
  CategoryLineupSchema,
  'id' | 'created_at' | 'updated_at'
> & {
  id?: string; // Optional for inserts
};

/**
 * Type for UPDATE operations (all fields optional except ID)
 */
export type CategoryLineupUpdate = {
  id: string;
} & Partial<Omit<CategoryLineupSchema, 'id' | 'created_at' | 'updated_at'>>;
