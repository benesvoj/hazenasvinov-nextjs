...
export function useSponsorshipData() {
  ...
  // Create main partner
  const createMainPartner = useCallback(
    async (partnerData: Omit<MainPartner, 'id' | 'created_at' | 'updated_at'>) => {
      try {
        const response = await fetch('/api/sponsorship/main-partners', {
          method: 'POST',
  ...
  const createBusinessPartner = useCallback(
    async (partnerData: Omit<BusinessPartner, 'id' | 'created_at' | 'updated_at'>) => {
      try {
        const response = await fetch('/api/sponsorship/business-partners', {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
  ...
  const createMediaPartner = useCallback(
    async (partnerData: Omit<MediaPartner, 'id' | 'created_at' | 'updated_at'>) => {
      try {
        const response = await fetch('/api/sponsorship/media-partners', {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
  ...
  // Update main partner
  const updateMainPartner = useCallback(
    async (id: string, partnerData: Partial<MainPartner>) => {
      try {
  ...
  // Delete main partner
  const deleteMainPartner = useCallback(
    async (id: string) => {
      try {
        const response = await fetch(`/api/sponsorship/main-partners/${id}`, {
          method: 'DELETE',
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      setLoading(true);
      try {
        await Promise.all([fetchMainPartners(), fetchBusinessPartners(), fetchMediaPartners()]);
      } catch (err) {
