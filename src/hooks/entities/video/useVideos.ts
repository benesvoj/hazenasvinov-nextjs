...
export function useVideos(options: UseVideosOptions = {}) {
  ...
  const fetchVideos = useCallback(
    async (filters: VideoFilters = {}, page: number = 1) => {
      try {
        setLoading(true);
        ...
        if (enableAccessControl && assignedCategories.length > 0) {
          ...
          // If access control enabled but no category assigned, show no videos
          setVideos([]);
          setTotalCount(0);
          setLoading(false);
          return;
        ...
        setError(
          `Chyba při načítání videí: ${err instanceof Error ? err.message : 'Neznámá chyba'}`
        );
      } finally {
        setLoading(false);
      }
  ...
  const updateVideo = useCallback(
    async (id: string, formData: VideoFormData) => {
      try {
        const supabase = createClient();
