...
export function useCategoryLineups() {
    ...
    async (categoryId: string, seasonId: string) => {
      ...
      try {
        ...
        if (
          ...
          err.message.includes('relation "category_lineups" does not exist')
        ) {
          console.warn(
            'Category lineups table does not exist. Please run the database setup script.'
          );
  ...
  const fetchLineupMembers = useCallback(
    async (lineupId: string) => {
      try {
        ...
        const members: CategoryLineupMember[] = (data || []).map(
          (record: RawCategoryLineupMember) => ({
            id: record.id,
        ...
        if (
          err instanceof Error &&
          err.message.includes('relation "category_lineup_members" does not exist')
        ) {
          console.warn(
            'Category lineup members table does not exist. Please run the database setup script.'
          );
          setError('Database tables not set up. Please contact administrator.');
  ...
  const createLineup = useCallback(
    async (lineupData: CategoryLineupFormData) => {
  ...
  const updateLineup = useCallback(
    async (id: string, lineupData: Partial<CategoryLineupFormData>) => {
      try {
        setError(null);
  ...
  const addMemberToLineup = useCallback(
    async (lineupId: string, memberData: AddMemberToLineupData) => {
      if (!user?.id) throw new Error('User not authenticated');
  ...
  const removeMemberFromLineup = useCallback(
  ...
  const updateLineupMember = useCallback(
    async (memberId: string, memberData: Partial<AddMemberToLineupData>) => {
      try {
        setError(null);
