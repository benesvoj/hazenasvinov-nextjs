...
export const useFetchCategoryLineupMembers = (categoryId: string, categoryLineupId: string) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        API_ROUTES.categories.lineupByIdMembers(categoryId, categoryLineupId)
      );
