...
import {CategoryLineupMember, CreateCategoryLineupMember, UpdateCategoryLineupMember} from "@/types";
...
export const useCategoryLineupMember = () => {
	...
	const createLineupMember = useCallback(async (
		categoryId: string,
		lineupId: string,
		data: CreateCategoryLineupMember
	) => {
		...
		try {
			...
			// Update local state WHYYYYYY?????
			setLineupMembers((prev) => [...prev, response.data]);
			return response;
		} catch (error) {
	...
	const updateLineupMember = useCallback(async (
			categoryId: string,
			lineupId: string,
			memberId: string,
			...
			try {
				...
				const res = await fetch(API_ROUTES.categories.lineupByIdMemberById(categoryId, lineupId, memberId), {
					method: 'PATCH',
					headers: {'Content-Type': 'application/json'},
					body: JSON.stringify(data),
				});
				...
				// Update local state WHYYYYYY?????
				setLineupMembers((prev) => prev.map((member) => member.id === memberId ? response.data : member));
				return response;
			} catch (error) {
	...
	const removeLineupMember = useCallback(async (
			categoryId: string,
			lineupId: string,
			memberId: string,
		) => {
