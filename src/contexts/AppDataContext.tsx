...
export function AppDataProvider({children}: {children: React.ReactNode}) {
  ...
  const activeSeason = useMemo(
    () => seasonsData.find((s: Season) => s.is_active) || null,
    [seasonsData]
  );
  ...
  // Global error state
  const error = useMemo(() => {
    const errorMessages = [seasonsError, categoriesError, membersError, clubsError].filter(
      (e) => e !== null
    );
  ...
  const refreshAll = async () => {
    await Promise.all([seasonsRefetch(), categoriesRefetch(), membersRefetch(), clubsRefetch()]);
  };
