...
├── useRecordings() → ALL videos (global fetch)
├── useRecordingFilter() → client-side filter (search, category, club, season, active)
│
...
- Fetches ALL videos globally via `useRecordings()` — no server-side category restriction
- Client-side filtering via `useRecordingFilter()` hook when coach selects a category filter
...
1. **No server-side category filtering** — `useRecordings()` returns ALL videos in the system. Client-side filtering restricts what's displayed, but all data is loaded into the browser. A coach could inspect network responses to see videos from unauthorized categories.
