      ...
      <ModalContent>
        ...
        <ModalBody>
          <div className="space-y-6">
            ...
            {/* Member Selection */}
                <h3 className="text-lg font-semibold mb-4">Vyberte člena</h3>
                {membersLoading ? (
                ...
                <CardBody>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Select
                      ...
                      isRequired
                    >
                      <SelectItem key="goalkeeper">
                        Brankář
                      </SelectItem>
                      <SelectItem key="field_player">
                        Hráč v poli
                      </SelectItem>
                    </Select>
                    ...
                    <Select
                      ...
                      onSelectionChange={(keys) => 
                        setJerseyNumber(Array.from(keys)[0] as string)
                      }
                  ...
                  <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-sm text-gray-600">
                      <div><strong>Jméno:</strong> {selectedMemberData.name} {selectedMemberData.surname}</div>
                      <div><strong>Reg. číslo:</strong> {selectedMemberData.registration_number}</div>
                      <div><strong>Pozice:</strong> {position === 'goalkeeper' ? 'Brankář' : 'Hráč v poli'}</div>
                      {jerseyNumber && <div><strong>Číslo dresu:</strong> {jerseyNumber}</div>}
                      {(isCaptain || isViceCaptain) && (
                        <div><strong>Funkce:</strong> {isCaptain ? 'Kapitán' : 'Zástupce kapitána'}</div>
                      )}
                    </div>
                  </div>
                </CardBody>
