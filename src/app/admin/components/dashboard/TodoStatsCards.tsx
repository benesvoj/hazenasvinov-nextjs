...
export const TodoStatsCards = ({todoFilter, setTodoFilter, stats}: TodoFilteringProps) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <Card
        className={`cursor-pointer transition-all hover:shadow-lg ${todoFilter === TodoFilter.TODO ? 'ring-2 ring-blue-500 bg-blue-50' : ''}`}
      >
        <CardBody
      ...
      <Card
        className={`cursor-pointer transition-all hover:shadow-lg ${todoFilter === TodoFilter.IN_PROGRESS ? 'ring-2 ring-orange-500 bg-orange-50' : ''}`}
      >
        <CardBody
          className="text-center"
          onClick={() =>
            setTodoFilter(
              todoFilter === TodoFilter.IN_PROGRESS ? TodoFilter.ALL : TodoFilter.IN_PROGRESS
            )
          }
        >
          <div className="flex items-center justify-center gap-2 mb-2">
      ...
      <Card
        className={`cursor-pointer transition-all hover:shadow-lg ${todoFilter === TodoFilter.DONE ? 'ring-2 ring-green-500 bg-green-50' : ''}`}
      >
        <CardBody
          className="text-center"
          onClick={() =>
      ...
      <Card
        className={`cursor-pointer transition-all hover:shadow-lg ${todoFilter === TodoFilter.HIGH_PRIORITY ? 'ring-2 ring-red-500 bg-red-50' : ''}`}
      >
        <CardBody
          className="text-center"
          onClick={() =>
