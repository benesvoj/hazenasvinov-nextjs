    ...
    <div className="space-y-6">
                ...
                <TableRow>
                  <TableCell colSpan={4} className="text-center py-8">
                    <div className="text-gray-500">
                      ...
                      <p className="text-sm">
                        Ujistěte se, že máte nastavené uživatelské role v databázi
                      </p>
                    </div>
                  </TableCell>
                </TableRow>
                  ...
                  <TableRow key={user.user_id}>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        {(user.roles || []).length > 0 ? (
                          (user.roles || []).map((role) => (
                            <Chip key={role} size="sm" color={getRoleColor(role)} variant="solid">
                              {getRoleLabel(role)}
                            </Chip>
                          ))
                        ) : (
                      ...
                      <div className="flex flex-wrap gap-1">
                        {(user.assigned_category_names || []).length > 0 ? (
                          (user.assigned_category_names || []).map((categoryName, index) => (
                            <Chip key={index} size="sm" color="secondary" variant="bordered">
                              {categoryName}
                            </Chip>
                          ))
                        ) : (
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} size="2xl" scrollBehavior="inside">
        <ModalContent>
              ...
              <div>
                ...
                >
                  <Checkbox value="admin">
                    Administrátor - plný přístup do admin i trenérského portálu
                  </Checkbox>
          ...
          <ModalFooter>
            <Button variant="bordered" onPress={closeModal} disabled={saving}>
              Zrušit
            </Button>
