...
export default function MembersStatisticTab({members, categoriesData}: MembersStatisticTabProps) {
  ...
  const statistics = useMemo(() => {
    ...
    // Function statistics
    const functionStats = getMemberFunctionOptions().reduce(
      (acc, {value, label}) => {
        const count = members.filter((m) => m.functions && m.functions.includes(value)).length;
        if (count > 0) {
          acc[label] = count;
        }
        return acc;
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
          <CardBody className="text-center">
            ...
            <p className="text-4xl font-bold text-purple-600">
              {statistics.ageStats.averageAge} let
            </p>
          </CardBody>
        </Card>
        ...
        <CardBody>
          {Object.keys(statistics.categoryStats).length > 0 ? (
            ...
            <p className="text-gray-500 text-center py-4">
              Žádné členové v kategoriích momentálně.
            </p>
          )}
        </CardBody>
      </Card>
