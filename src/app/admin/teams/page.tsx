...
export default function TeamsAdminPage() {
  ...
  const handleAddAllCategories = async () => {
    ...
    try {
      ...
      const {error} = await supabase.from('team_categories').insert(categoriesData);
        ...
        <CardBody>
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {teams
                .filter(
                  (team) => selectedCommittee === 'all' || team.committee_id === selectedCommittee
                )
                .map((team) => (
                  <div
                    key={team.id}
                  ...
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3
                            className={`text-lg font-semibold ${team.is_own_club ? 'text-green-800 dark:text-green-200' : ''}`}
                          >
                            {team.name}
                          </h3>
                          {team.short_name && (
                            <p
                              className={`text-sm ${team.is_own_club ? 'text-green-600 dark:text-green-300' : 'text-gray-600 dark:text-gray-400'}`}
                            >
                              {team.short_name}
                            </p>
                          )}
                          {team.is_own_club && (
                            <Badge color="success" variant="solid" size="sm" className="mt-1">
                              ⭐ Domovský klub
                            </Badge>
                          )}
                        </div>
                ...
                <div className="space-y-3 pt-2">
                  ...
                  {(logoPreview || formData.logo_url) && (
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <Button size="sm" color="danger" variant="flat" onPress={clearLogoPreview}>
                        Odstranit
                      </Button>
                    </div>
                  )}
                    ...
                    <div className="space-y-3">
                      ...
                      {teamCategories.length === 0 ? (
                        <p className="text-gray-500 text-center py-4">
                          Žádné kategorie nebyly přiřazeny
                        </p>
                      ) : (
                              ...
                              {teamCategories.map((teamCategory) => (
                                ...
                                >
                                  <td className="py-2 px-3">
                                    <Badge
                                      color={getCategoryBadgeColor(teamCategory.category)}
                                      variant="flat"
                                    >
