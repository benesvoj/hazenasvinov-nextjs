...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!email || !status || !action) {
      return NextResponse.json({error: 'Email, status, and action are required'}, {status: 400});
    }
    ...
    if (!['login', 'logout'].includes(action)) {
      return NextResponse.json({error: 'Action must be either "login" or "logout"'}, {status: 400});
    }
    ...
    if (!isProduction || isLocalhost) {
      ...
      console.log(
        `[DEV] Login action would be logged: ${email} - ${action} - ${status} (skipped in development)`
      );
      return NextResponse.json({
        success: true,
