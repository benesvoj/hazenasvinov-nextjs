...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      debug_info: {
        all_posts_sample: allPosts?.slice(0, 3).map(p => ({ id: p.id, title: p.title, status: p.status })) || [],
        published_sample: publishedPosts?.slice(0, 3).map(p => ({ id: p.id, title: p.title, status: p.status })) || []
      }
    });
