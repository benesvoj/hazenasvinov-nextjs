    ...
    <Card key={meeting.id} className="hover:shadow-lg transition-shadow">
      <CardBody>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div>
            <Heading size={4}>{t.meetingDetails}</Heading>
            <div className="space-y-2 text-sm">
              <div className="flex items-center gap-2">
                <UserIcon className="w-4 h-4 text-gray-400" />
                <span>
                  {t.wroteBy}:{' '}
                  {meeting.wrote_by_user?.user_metadata?.full_name || meeting.wrote_by_user?.email}
                </span>
              </div>
                  ...
                  <div className="flex gap-1">
                    ...
                    <Button
                      ...
                      onPress={() =>
                        handleDownload(
                          meeting.attachment_url!,
                          meeting.attachment_filename || 'attachment'
                        )
                      }
          ...
          <div>
            <Heading size={4}>{t.attendanceList}</Heading>
            {meeting.attendees && meeting.attendees.length > 0 ? (
              <div className="space-y-3">
                ...
                <div className="pt-2 border-t text-xs text-gray-500">
                  {t.totalAttendees}: {meeting.attendees.length} |{t.presentCount}:{' '}
                  {meeting.attendees.filter((a) => a.status === 'present').length} |{t.excusedCount}
                  : {meeting.attendees.filter((a) => a.status === 'excused').length}
                </div>
              </div>
            ) : (
