    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-gray-600">{filteredData.length} záznamů</span>
        </div>
      </div>
      ...
      <div className="block lg:hidden">
        {paginatedData.length === 0 ? (
          ...
          <div className="space-y-3">{paginatedData.map(renderMobileCard)}</div>
        )}
      </div>
      ...
      <div className="hidden lg:block">
        ...
        >
          ...
          <TableBody items={paginatedData} emptyContent={emptyMessage}>
            {(item) => (
              <TableRow
                key={JSON.stringify(item)}
              ...
              >
                {(columnKey) => {
                  ...
                  return <TableCell key={String(columnKey)}>{renderCell(item, column)}</TableCell>;
                }}
              </TableRow>
            )}
          </TableBody>
