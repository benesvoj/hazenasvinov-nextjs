...
export function HorizontalNavigation({items, className = '', onItemClick}: NavigationProps) {
          ...
          {item.children ? (
            <div className="relative">
              ...
              >
                ...
                {item.badge && (
                  <span
                    className={`ml-2 px-2 py-1 text-xs rounded-full bg-${item.badge.color || 'primary'}-100 text-${item.badge.color || 'primary'}-800 dark:bg-${item.badge.color || 'primary'}-900/20 dark:text-${item.badge.color || 'primary'}-400`}
                  >
                    {item.badge.text}
                  </span>
                )}
                  ...
                  <div className="py-2">
                    {item.children.map((child) => (
                      ...
                      >
                        ...
                        {child.badge && (
                          <span
                            className={`ml-auto px-2 py-1 text-xs rounded-full bg-${child.badge.color || 'primary'}-100 text-${child.badge.color || 'primary'}-800 dark:bg-${child.badge.color || 'primary'}-900/20 dark:text-${child.badge.color || 'primary'}-400`}
                          >
                            {child.badge.text}
                          </span>
                        )}
            ...
            >
              ...
              {item.badge && (
                <span
                  className={`px-2 py-1 text-xs rounded-full bg-${item.badge.color || 'primary'}-100 text-${item.badge.color || 'primary'}-800 dark:bg-${item.badge.color || 'primary'}-900/20 dark:text-${item.badge.color || 'primary'}-400`}
                >
                  {item.badge.text}
                </span>
              )}
...
// Vertical Navigation
export function VerticalNavigation({items, className = '', onItemClick}: NavigationProps) {
  ...
  const renderItem = (item: NavigationItem, level: number = 0) => (
    <div key={item.title}>
      ...
      {item.children && openItems.has(item.title) && (
        <div className="mt-1">{item.children.map((child) => renderItem(child, level + 1))}</div>
      )}
    </div>
  );

  return <nav className={`space-y-1 ${className}`}>{items.map((item) => renderItem(item))}</nav>;
}

// Mobile Navigation
export function MobileNavigation({items, isOpen, onClose, onItemClick}: MobileNavigationProps) {
  const [openItems, setOpenItems] = useState<Set<string>>(new Set());
    ...
    <div key={item.title}>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          {item.badge && (
            <span
              className={`px-3 py-1 text-xs rounded-full bg-${item.badge.color || 'primary'}-100 text-${item.badge.color || 'primary'}-800 dark:bg-${item.badge.color || 'primary'}-900/20 dark:text-${item.badge.color || 'primary'}-400`}
            >
              {item.badge.text}
            </span>
          )}
    ...
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      <div className="fixed inset-0 bg-white dark:bg-gray-900 shadow-xl flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto relative">
          ...
          <div className="py-2">{items.map((item) => renderMobileItem(item))}</div>
        </div>
    ...
    <nav className={`flex items-center space-x-2 text-sm ${className}`}>
      {items.map((item, index) => (
        <div key={index} className="flex items-center">
          {index > 0 && <span className="mx-2 text-gray-400 dark:text-gray-600">{separator}</span>}
